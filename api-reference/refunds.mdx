...
```bash
curl -X GET "https://api.rollapay.com/transactions/{id}/refundable?organizationId={orgId}" \
...
<Card title="POST /transactions/:id/refund" icon="rotate-left">
...
curl -X POST "https://api.rollapay.com/transactions/{id}/refund?organizationId={orgId}" \
  -H "Authorization: Bearer sk_live_your_secret_key" \
...
const response = await fetch(
  `https://api.rollapay.com/transactions/${transactionId}/refund?organizationId=${orgId}`,
  {
    method: 'POST',
    headers: {
      'Authorization': 'Bearer sk_live_your_secret_key',
...
response = requests.post(
    f'https://api.rollapay.com/transactions/{transaction_id}/refund',
...
<RequestExample>
```bash cURL
curl -X GET "https://api.rollapay.com/refunds?organizationId={orgId}&status=completed&limit=10" \
  -H "Authorization: Bearer sk_live_your_secret_key"
```
</RequestExample>
...
<RequestExample>
```bash cURL
curl -X GET "https://api.rollapay.com/refunds/{refundId}?organizationId={orgId}" \
  -H "Authorization: Bearer sk_live_your_secret_key"
```
</RequestExample>
